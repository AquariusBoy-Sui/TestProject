...
@property (nonatomic, strong) NSMutableArray<UIImageView *> *lines;
...
- (void)configTextField
{
    ...
    maskView.backgroundColor = [UIColor getSystemBackgroundColorNormalWhite];
    ...
    for (NSInteger i = 0; i < self.itemCount; i++)
    {
        ...
        label.textColor = [UIColor getGrayLabelColor];
    ...
    for (NSInteger i = 0; i < self.itemCount; i++)
    {
        UIImageView *line = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bindtele_short_line"]];
...
- (void)tfEditingChanged:(UITextField *)textField
{
    ...
    if (textField.text.length >= self.itemCount) {
        ...
        if (self.deleagete && [self.deleagete respondsToSelector:@selector(inputCompletion:)]) {
            [self.deleagete inputCompletion:textField.text];
        }
