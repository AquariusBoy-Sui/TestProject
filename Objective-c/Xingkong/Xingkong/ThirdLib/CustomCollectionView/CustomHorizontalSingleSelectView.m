...
@property(nonatomic, copy)NSString *selectImgStr;
...
- (void)setSelectIndex:(NSInteger)selectIndex {
    ...
    for (int i = 0; i < _imageArr.count; i ++) {
        if (_selectIndex == i) {
            ...
            self.selectImgStr = model.imgStr;
        }
...
- (void)clickBtn:(CustomCollectionViewCell *)cell {
    ...
    _selectImgStr = model.imgStr;
    }
    if (self.Delegate && [self.Delegate respondsToSelector:@selector(clickBtnHorizontalSingle:select:selectImgStr:)]) {
        [self.Delegate clickBtnHorizontalSingle:indexPath.item select:_selectImgid selectImgStr:_selectImgStr];
    }
}
@end
