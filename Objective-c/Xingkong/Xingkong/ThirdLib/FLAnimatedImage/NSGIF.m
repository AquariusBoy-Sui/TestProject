...
+ (void)optimalGIFfromURL:(NSURL*)videoURL withFrameCount:(int)cframeCount  loopCount:(int)loopCount completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    float frameCount =videoLength*framesPerSecond;
    ...
    for (int currentFrame = 0; currentFrame<cframeCount; ++currentFrame) {
...
+ (NSURL *)createGIFforTimePoints:(NSArray *)timePoints fromURL:(NSURL *)url fileProperties:(NSDictionary *)fileProperties frameProperties:(NSDictionary *)frameProperties frameCount:(int)frameCount gifSize:(GIFSize)gifSize{
    ...
    NSString *temporaryFile = [kCachesPath stringByAppendingPathComponent:[NSString stringWithFormat:@"/%@-%@.gif", fileName,[NSUUID uuidObjectID]]];
