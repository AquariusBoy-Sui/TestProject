...
- (void)initDefaultConfign{
    ...
    _contentNorColor = [UIColor getlabelColor];
...
- (void)setContentView:(NSArray *)contenArr titleArr:(NSArray *)titleArr imgArr:(NSArray*)imgNames{
    //这里第一次传入图片数组
    self.imgArr=imgNames;
    ...
    for (NSInteger i = 0 ; i < titleArr.count; ++i) {
        //设置每一组的值，并返回最后一个frame
         @autoreleasepool {
             self.frameRect = [self setSignView:contenArr[i] andTitle:titleArr[i] andFrame:self.frameRect andGroupId:i andImage:self.imgArr[i]];
         }
    }
    //设置滚动视图的滚动范围
...
- (CGRect)setSignView:(NSArray *)dataAr andTitle:(NSString *)titleStr andFrame:(CGRect)frame andGroupId:(NSInteger)groupId andImage:(NSArray*)imgNamest{
//这里第二次传入图片数组
    ...
    for (NSInteger i = 0; i < dataAr.count; ++i) {
            ...
            [but setTitleEdgeInsets:UIEdgeInsetsMake(0, -22, 0, 0)];
            //加载图片数组的方法 不确定是不是加在这里
//        NSLog(@"%@",imgNamest);
            [but setImage:[UIImage imageNamed:imgNamest[i]] forState:UIControlStateNormal];
            [but setImageEdgeInsets:UIEdgeInsetsMake(7, -17, 7, 0)];
            ...
            //处理标签流
        //宽度在这修改
            CGFloat butWidth = contentSize.width + 45;
...
- (void)reset{
    ...
    [self setContentView:_contetntArr titleArr:_titleArr imgArr:_imgArr];
}
...
- (UIScrollView *)scroller{
    if (!_scroller) {
        ...
        //
        _scroller.backgroundColor = [UIColor getSystemBackgroundColorNormalWhite];
